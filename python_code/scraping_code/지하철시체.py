...
# 쿠키닷컴 게시글 사이트 주소
URL = "https://gall.dcinside.com/board/view/?id=hit&no=1&page=1"
# 크롬드라이버 경로 설정
...
def get_last_page(URL):
    ...
    n = int(last_page.get_attribute('href')[-7:-5])
    return n
        ...
        for comment in comments:
            if comment.get("class") == ["ub-content"]:
                ...
                if comment.find(class_="del_reply") is None:
                    nick = "에러_발생" if comment.find("div","cmt_nickbox").text == None else comment.find("div","cmt_nickbox").text
                    nicks.append(nick)
                    usertxt = "에러_발생" if comment.select_one("div.clear.cmt_txtbox").text == None else comment.select_one("div.clear.cmt_txtbox").text
...
comment_dics = scrapping_comments(URL)
df = save_data(comment_dics,"지하철시체")